...
// TestSessionsList_MultipleFilters guards against a regression where two or more
// WHERE conditions were joined with a bare space instead of " AND ", producing
// invalid SQL (e.g. `WHERE s.project IN (?) s.started_at >= ?`) and a 500 error.
func TestSessionsList_MultipleFilters(t *testing.T) {
	db := openSessionTestDB(t)
	seedSessionData(t, db)

	// proj-a sessions: s1 (started 2026-01-10), s2 (started 2026-01-11).
	// Combining a project filter with a from-date must yield only s2.
	...
	if err != nil {
		t.Fatalf("SessionsList with project+from filters: %v", err)
	}
	if len(result.Items) != 1 {
		t.Fatalf("expected 1 session (s2), got %d", len(result.Items))
	}
	if result.Items[0].ID != "s2" {
		t.Errorf("expected s2, got %q", result.Items[0].ID)
	}
}
...
// TestSessionsList_Enrich verifies the batched enrichment (tags + recent_title)
// produces the same result the old per-session N+1 did, and exercises the
// window-function queries against modernc.org/sqlite.
func TestSessionsList_Enrich(t *testing.T) {
	db := openSessionTestDB(t)
	for _, stmt := range []string{
		`INSERT INTO sessions (id, project, directory, started_at) VALUES ('s-enr', 'p', '/p', '2026-02-01 10:00:00')`,
		`INSERT INTO observations (session_id, type, title, content, project, topic_key, created_at, updated_at) VALUES ('s-enr', 'note', 'First', 'c', 'p', 'topic/a', '2026-02-01 10:05:00', '2026-02-01 10:05:00')`,
		`INSERT INTO observations (session_id, type, title, content, project, topic_key, created_at, updated_at) VALUES ('s-enr', 'note', 'Second', 'c', 'p', 'topic/a', '2026-02-01 10:10:00', '2026-02-01 10:10:00')`,
		`INSERT INTO observations (session_id, type, title, content, project, topic_key, created_at, updated_at) VALUES ('s-enr', 'note', 'Third', 'c', 'p', 'topic/b', '2026-02-01 10:15:00', '2026-02-01 10:15:00')`,
	} {
		if _, err := db.Exec(stmt); err != nil {
			t.Fatalf("seed: %v", err)
		}
	...
	result, err := services.SessionsList(db, services.SessionListParams{Limit: 10, Enrich: true})
	if err != nil {
		t.Fatalf("SessionsList enrich: %v", err)
	}

	var item *services.SessionListItem
	for i := range result.Items {
		if result.Items[i].ID == "s-enr" {
			item = &result.Items[i]
		}
	}
	if item == nil {
		t.Fatal("session s-enr not found")
	}
	// topic/a has frequency 2, topic/b has frequency 1 → ordered a, b.
	if len(item.Tags) != 2 || item.Tags[0] != "topic/a" || item.Tags[1] != "topic/b" {
		t.Errorf("tags: got %v, want [topic/a topic/b]", item.Tags)
	}
	// Most recent observation (10:15) is "Third".
	if item.RecentTitle == nil || *item.RecentTitle != "Third" {
		t.Errorf("recent_title: got %v, want Third", item.RecentTitle)
	}
}
