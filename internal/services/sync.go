...
// projectEntityUnionSQL is shared across services.
const projectEntityUnionSQL = `
...
// readProjectAggregates fetches the per-project aggregates.
func readProjectAggregates(db *sql.DB) ([]projectAggregate, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&a.project, &a.obsCount, &a.sessionsCount, &a.promptsCount, &a.lastActivity); err != nil {
			return nil, err
		}
...
func SyncGetProjectDetail(db *sql.DB, project string) (*SyncProjectDetailResponse, error) {
	...
	defer rows.Close()
	pending := []SyncMutationRow{}
	for rows.Next() {
		var m SyncMutationRow
		if err := rows.Scan(&m.Seq, &m.TargetKey, &m.Entity, &m.EntityKey, &m.Op, &m.Source, &m.OccurredAt, &m.AckedAt, &m.Project); err != nil {
			return nil, err
		}
...
func SyncComputeIssues(db *sql.DB, daemonAvailable bool) (SyncIssuesResponse, error) {
	...
	for _, p := range data.Projects {
		...
		if p.Status == SyncStatusBroken && p.Lifecycle != nil && *p.Lifecycle == "pending" && ack == 0 && enq > 0 {
			hint := "Check `last_error` / `reason_message` and verify cloud token / connectivity"
			issues = append(issues, SyncIssue{
				Code:     "SYNC_BROKEN",
				Severity: SyncIssueSeverityHigh,
				Message:  p.Project + ": " + itoa64(enq) + " mutations enqueued, 0 acked — sync is silently broken",
				Project:  &pp.Project,
				Hint:     &hint,
