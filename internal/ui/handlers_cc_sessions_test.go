...
// ---------------------------------------------------------------------------
// Tests: GET /cc-overview (Overview tab — default)
// ---------------------------------------------------------------------------

func TestCCOverviewPage_FullPage(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/cc-overview", nil)
	...
	// Overview tab must be active (aria-current="page").
...
// ---------------------------------------------------------------------------
// Tests: GET /cc-overview?tab=sessions (Sessions tab)
// ---------------------------------------------------------------------------

func TestCCOverviewSessionsTab_FullPage(t *testing.T) {
	base := buildCCTestDir(t, "my-project-folder", "sess-001",
		minimalJSONL("e:/my-project", "Hello Claude"))
	...
	req := httptest.NewRequest(http.MethodGet, "/cc-overview?tab=sessions", nil)
	w := httptest.NewRecorder()
	mux.ServeHTTP(w, req)
	...
	if !strings.Contains(body, "sess-001") {
		t.Errorf("sessions tab must contain session id; body snippet: %s", body[:min(500, len(body))])
	}
}

func TestCCOverviewSessionsTab_HTMXPartial(t *testing.T) {
...
func TestCCOverviewSessionsTab_NonexistentDir(t *testing.T) {
...
func TestCCOverviewSessionsTab_ProjectFilter(t *testing.T) {
	base := t.TempDir()
	...
	req := httptest.NewRequest(http.MethodGet, "/cc-overview?tab=sessions&project=alpha", nil)
	w := httptest.NewRecorder()
	mux.ServeHTTP(w, req)
	...
	if !strings.Contains(body, "alpha") {
		t.Error("filtered sessions tab must show alpha project")
	}
	if strings.Contains(body, "beta") {
		t.Error("filtered sessions tab must NOT show beta project")
	}
}

// ---------------------------------------------------------------------------
// Tests: GET /cc-sessions redirect
// ---------------------------------------------------------------------------

// TestCCSessionsRedirect verifies GET /cc-sessions returns 301 → /cc-overview?tab=sessions.
func TestCCSessionsRedirect(t *testing.T) {
	mux := http.NewServeMux()
	...
	if loc != "/cc-overview?tab=sessions" {
		t.Errorf("expected redirect to /cc-overview?tab=sessions, got %q", loc)
	}
}

// ---------------------------------------------------------------------------
// Tests: GET /cc-sessions/list (HTMX partial — unchanged)
// ---------------------------------------------------------------------------
