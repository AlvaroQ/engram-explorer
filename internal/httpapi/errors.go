...
// writeDBError is a convenience wrapper for the most common database-error
// response pattern: HTTP 500, code "INTERNAL", message "database error", with
// err.Error() as the detail shown only when exposeDetails is true.
func writeDBError(w http.ResponseWriter, err error, exposeDetails bool) {
	writeError(w, http.StatusInternalServerError, "INTERNAL", "database error", err.Error(), exposeDetails)
}
