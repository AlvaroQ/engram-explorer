...
/// Returns the API base URL for the current runtime environment.
///
/// Production (single-binary mode): the React SPA is served by the Go binary on
/// the same origin, so all `/api/*` fetches are same-origin.  `__ENGRAM_API_BASE__`
/// is undefined and we return `""` — no proxy, no CORS, no special config needed.
///
/// Development (Vite dev server): `__ENGRAM_API_BASE__` is still undefined and we
/// return `""`.  Vite's `/api` proxy in vite.config.ts forwards requests to the Go
/// binary at 127.0.0.1:8787.
///
/// Legacy Tauri desktop: the Rust shell injects `window.__ENGRAM_API_BASE__`
/// (e.g. "http://127.0.0.1:8787") via an initialization script.  The desktop app
/// is being removed; this path is kept for backward compatibility only.
function apiBase(): string {
  return (globalThis as { __ENGRAM_API_BASE__?: string }).__ENGRAM_API_BASE__ ?? '';
}
...
export const api = {
  ...
  assignProject: (entity: 'observation' | 'session' | 'prompt', id: string | number, project: string) =>
    request<AssignProjectResponse>(`/api/${entity}/${encodeURIComponent(String(id))}/project`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ project }),
    }),
  deleteEntity: (entity: 'observation' | 'session' | 'prompt', id: string | number) =>
    request<DeleteEntityResponse>(`/api/${entity}/${encodeURIComponent(String(id))}`, {
      method: 'DELETE',
    }),
  ...
  exportDb: async (): Promise<Blob> => {
    const res = await fetch(`${apiBase()}/api/db/export`, { headers: { Accept: 'application/octet-stream' } });
    if (!res.ok) {
      let payload: { error?: ApiError } = {};
      try {
        payload = (await res.json()) as { error?: ApiError };
