...
import { tween, easeOutCubic, DURATIONS, useReducedMotion } from './motion/interpolator.ts';
...
interface BrainLevelCrossFadeProps extends Omit<
  BrainLevelMeshProps,
  'opacityRef' | 'animateEntrance'
> {
  levelKey: string;
}
...
export function BrainLevelNodes({
  ...
  const prevStateRef = useRef<CrossFadeState>({
    levelKey: '',
    level: null,
    positions: null,
    animateEntrance: false,
...
function NodeHalo({
  ...
  return <HaloMesh x={x} y={y} z={z} radius={radius} color={color} opacity={opacity} />;
}
