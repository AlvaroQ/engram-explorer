...
import { useReducedMotion } from '../../features/brain/motion/interpolator.ts'
...
export function ProjectTitleOverlay({ nodes, positions, project, subtle = false }: ProjectTitleOverlayProps) {
  ...
  return (
    <Html position={anchor} center zIndexRange={[30, 0]} style={{ pointerEvents: 'none' }}>
      ...
      <div key={project} className={reducedMotion ? '' : 'animate-brain-title-float'}>
        <div
          ...
          style={{ borderColor: color, opacity: subtle ? 0.9 : 1, filter: subtle ? 'brightness(0.55)' : undefined }}
        >
