...
import { TweenDriver } from '../../features/brain/motion/use-tween.ts'
import { tween, easeOutExpo, DURATIONS, useReducedMotion } from '../../features/brain/motion/interpolator.ts'
import { motionStore } from '../../features/brain/motion/motion-store.ts'
import type { BrainModel, BrainLevel, BrainNode } from '../../features/brain/model/types.ts'
import type { BrainNavigation } from '../../features/brain/navigation/use-brain-navigation.ts'
...
function CameraFocus({
  ...
  useEffect(() => {
    ...
    if (focusNodeId != null) {
      ...
      const idxs: number[] = []
      nodes.forEach((node, i) => { if (matchedIds.has(node.id)) idxs.push(i) })
      if (idxs.length > 0) {
  ...
  }, [focusNodeId, focusInsetRight, matchedIds, positions, nodes, camera, controls, viewportWidth, viewportHeight, reducedMotion])
...
function LegacyScene({
  ...
  return (
    <>
      ...
      <ProjectTitleOverlay nodes={nodes} positions={positions} project={selected?.project ?? null} />
      ...
      {hovered?.project != null && hovered.project !== (selected?.project ?? null) && (
        <ProjectTitleOverlay nodes={nodes} positions={positions} project={hovered.project} subtle />
      )}
      <OrbitControls
        makeDefault
        enableDamping
...
export function GraphScene(props: Props) {
  ...
  if (!props.brainModel) {
    const { nodes, edges, debug = false, colorBy = 'project', onNodeClick, onNodeHover, onUserCameraStart, selected, hovered, matchedIds, focusNodeId, focusInsetRight, animate = true, onReady } = props
...
function BrainModelScene({
  ...
  useEffect(() => {
    ...
    const path = navigation.path
    const derivedLevel = path.length === 0
      ? brainModel.root({})
      : (() => {
          const top = path[path.length - 1]
          return top ? brainModel.children(top.id, {}) : brainModel.root({})
    ...
    if (derivedLevel.nodes.length === 0) {
      const next: CommittedLevel = { level: derivedLevel, positions: null, key: levelKey, burstOrigin: null, depth }
    ...
    const workerNodes = nodes.map((n, i) => ({
      ...
      subCluster: projectMap.get(typeof n.meta['project'] === 'string' ? n.meta['project'] : null) ?? 0,
    }))
    ...
    worker.onmessage = (e: MessageEvent<WorkerResult>) => {
      ...
      const next: CommittedLevel = { level: derivedLevel, positions, key: levelKey, burstOrigin, depth }
      committedRef.current = next
      setCommitted(next)
