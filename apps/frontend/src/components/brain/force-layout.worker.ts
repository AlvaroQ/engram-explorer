...
self.onmessage = (e: MessageEvent<WorkerMsg>) => {
  ...
  if (n > 0) {
    ...
    if (hasNonFinite) {
      console.warn('[force-layout] reset non-finite positions to origin (see force calc / NaN propagation)')
    }
    ...
    if (Number.isFinite(extent) && extent > 0.0001) {
      ...
      if (import.meta.env.DEV) {
        console.log(`[force-layout] normalized with scale=${scale.toFixed(3)}, target half=${TARGET_HALF}`)
      }
    } else {
